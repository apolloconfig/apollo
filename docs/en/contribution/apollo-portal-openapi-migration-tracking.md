...
| --- | --- | --- | --- |
| OpenAPI contract | `apollo-portal` points to the released `apollo-openapi` `v0.3.3` tag after the Permission/AccessKey slice | Portal implementation, generated interfaces, and SDKs can drift | Run compatibility checks before changing the spec URL, and pin a clear tag or commit |
...
- `apollo-openapi` `v0.3.3` is now the current adaptation target. Compared with `v0.1.0`, the `v0.3.x` line adds Release/Branch/Instance/Permission/AccessKey operations, removes or renames several `v0.1.0` paths, and tightens generated return types such as App create/update/delete, cluster delete, env cluster info, and missing envs.
- The current adaptation pass points the default POM URL to `apollo-openapi` `v0.3.3`. Remaining `v0.1.0` differences from the compatibility checker must stay documented as explicit compatibility exceptions or be handled by future aliases; they are not silently compatible.

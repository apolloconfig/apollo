...
| --- | --- | --- | --- |
| OpenAPI contract | `apollo-portal` still points to the `apollo-openapi` `v0.3.0` tag; the Release/Branch/Instance slice is validated with a local `v0.3.1` candidate spec until the tag exists | Portal implementation, generated interfaces, and SDKs can drift | Run compatibility checks before changing the spec URL, and pin a clear tag or commit |
| Frontend calls | See the [frontend URL migration inventory](./apollo-portal-openapi-frontend-url-inventory.md): 53 of the current 121 URL entries call OpenAPI, and 68 still call WebAPI | One-off migrations miss prefix path, SSO, permissions, and response shape details | Migrate by domain after backend dual-auth validation |
...
- The `v0.3.1` candidate spec adds `toReleaseId` to rollback and relaxes branch delete/rule-update `operator` query parameters so Portal USER requests can derive operators from the login session while token CONSUMER clients keep the existing payload/query operator behavior.
...
| Release / Branch | `ReleaseService.js`, `NamespaceBranchService.js` | release, gray release, branch create/delete/merge/rules, compare, active releases, and rollback are covered in the `v0.3.1` candidate | Frontend service calls are migrated; keep dual-auth and Java client compatibility coverage while waiting for the released spec tag |

...
public class ReleaseOpenApiServiceTest extends AbstractOpenApiServiceTest {
  ...
  public void testPublishNamespace() throws Exception {
    ...
    releaseOpenApiService.publishNamespace(
        someAppId, someEnv, someCluster, someNamespace, namespaceReleaseDTO);
  ...
  public void testRollbackRelease() throws Exception {
    ...
    assertEquals(
        String.format(
            "%s/envs/%s/releases/%s/rollback?operator=%s",
            someBaseUrl, someEnv, someReleaseId, someOperator),
        put.getURI().toString());
  }

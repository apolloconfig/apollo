...
public class ReleaseService {
  ...
  public ReleaseService(
      final UserInfoHolder userInfoHolder, final AdminServiceAPI.ReleaseAPI releaseAPI) {
  ...
  public ReleaseDTO publish(NamespaceReleaseModel model) {
    ...
    String releaseBy =
        StringUtils.isEmpty(model.getReleasedBy())
    ...
    Tracer.logEvent(
        TracerEventType.RELEASE_NAMESPACE,
  ...
  public ReleaseDTO publish(NamespaceGrayDelReleaseModel model, String releaseBy) {
    ...
    ReleaseDTO releaseDTO =
        releaseAPI.createGrayDeletionRelease(
            appId,
            env,
            clusterName,
            namespaceName,
  ...
  public ReleaseDTO updateAndPublish(
    ...
    return releaseAPI.updateAndPublish(
        appId,
        env,
        clusterName,
        namespaceName,
  ...
  public List<ReleaseBO> findAllReleases(
  ...
  public ReleaseCompareResult compare(ReleaseDTO baseRelease, ReleaseDTO toCompareRelease) {
    Map<String, String> baseReleaseConfiguration =
        baseRelease == null
            ? new HashMap<>()
            : GSON.fromJson(baseRelease.getConfigurations(), GsonType.CONFIG);
    Map<String, String> toCompareReleaseConfiguration =
        toCompareRelease == null
            ? new HashMap<>()
    ...
    for (Map.Entry<String, String> entry : baseReleaseConfiguration.entrySet()) {
      ...
      // added
      if (secondValue == null) {
        compareResult.addEntityPair(
            ChangeType.DELETED, new KVEntity(key, firstValue), new KVEntity(key, null));
      } else if (!Objects.equal(firstValue, secondValue)) {
        compareResult.addEntityPair(
            ChangeType.MODIFIED, new KVEntity(key, firstValue), new KVEntity(key, secondValue));
      }
    }
    ...
    for (Map.Entry<String, String> entry : toCompareReleaseConfiguration.entrySet()) {
      ...
      if (baseReleaseConfiguration.get(key) == null) {
        compareResult.addEntityPair(
            ChangeType.ADDED, new KVEntity(key, ""), new KVEntity(key, value));
      }
    }

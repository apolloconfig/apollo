...
public class ReleaseHistoryService {
  ...
  public ReleaseHistoryService(
      final ReleaseHistoryAPI releaseHistoryAPI,
  ...
  public ReleaseHistoryBO findLatestByReleaseIdAndOperation(
      Env env, long releaseId, int operation) {
  ...
  public ReleaseHistoryBO findLatestByPreviousReleaseIdAndOperation(
      Env env, long previousReleaseId, int operation) {
    PageDTO<ReleaseHistoryDTO> pageDTO =
        releaseHistoryAPI.findByPreviousReleaseIdAndOperation(
            env, previousReleaseId, operation, 0, 1);
    if (pageDTO != null && pageDTO.hasContent()) {
      ReleaseHistoryDTO releaseHistory = pageDTO.getContent().get(0);
      ReleaseDTO release = releaseService.findReleaseById(env, releaseHistory.getReleaseId());
      return transformReleaseHistoryDTO2BO(releaseHistory, release);
  ...
  public List<ReleaseHistoryBO> findNamespaceReleaseHistory(
      String appId, Env env, String clusterName, String namespaceName, int page, int size) {
    PageDTO<ReleaseHistoryDTO> result =
        releaseHistoryAPI.findReleaseHistoriesByNamespace(
            appId, env, clusterName, namespaceName, page, size);
  ...
  private List<ReleaseHistoryBO> transformReleaseHistoryDTO2BO(
      List<ReleaseHistoryDTO> source, List<ReleaseDTO> releases) {
    if (CollectionUtils.isEmpty(source)) {
      return Collections.emptyList();
    }
    this.additionalUserInfoEnrichService.enrichAdditionalUserInfo(
        source, BaseDtoUserInfoEnrichedAdapter::new);
  ...
  private ReleaseHistoryBO transformReleaseHistoryDTO2BO(
      ReleaseHistoryDTO dto, ReleaseDTO release) {

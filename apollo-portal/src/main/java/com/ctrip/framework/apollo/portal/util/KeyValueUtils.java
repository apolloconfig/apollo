...
public class KeyValueUtils {
  ...
  /**
   * make a filter on properties. and convert properties to a map the suffix match is case
   * insensitive
   *
   * @param properties
  ...
  /**
   * make a filter on map's key, keep the k-v which key ends with suffix given the suffix match is
   * case insensitive
   *
   * @param keyValues
   * @param suffix suffix in a key
  ...
  /**
   * remove key's suffix in a map suppose that all keys's length not smaller than suffixLength, if
   * not satisfied, a terrible runtime exception will occur
   *
   * @param keyValues

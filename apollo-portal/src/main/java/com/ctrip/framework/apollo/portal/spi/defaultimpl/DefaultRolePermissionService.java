...
public class DefaultRolePermissionService implements RolePermissionService {
  ...
  @Autowired private ConsumerRoleRepository consumerRoleRepository;
          ...
          permissionIds.stream()
              .map(
                  permissionId -> {
                    ...
                    rolePermission.setDataChangeLastModifiedBy(
                        createdRole.getDataChangeLastModifiedBy());
   ...
   */
  @Transactional
  public Set<String> assignRoleToUsers(
      String roleName, Set<String> userIds, String operatorUserId) {
    ...
    Iterable<UserRole> toCreate =
        toAssignUserIds.stream()
            .map(
                userId -> {
  ...
  public Set<Permission> createPermissions(Set<Permission> permissions) {
    ...
    for (String targetId : targetIdPermissionTypes.keySet()) {
      ...
      Preconditions.checkState(
          CollectionUtils.isEmpty(current),
          "Permission with permissionType %s targetId %s already exists!",
          permissionTypes,
          targetId);
    }
  ...
  @Transactional
  @Override
  public void deleteRolePermissionsByAppIdAndNamespace(

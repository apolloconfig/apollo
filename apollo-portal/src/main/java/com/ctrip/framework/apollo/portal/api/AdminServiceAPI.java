...
public class AdminServiceAPI {
  ...
  public static class NamespaceAPI extends API {
    ...
    public PageDTO<NamespaceDTO> findByItem(Env env, String itemKey, int page, int size) {
      ResponseEntity<PageDTO<NamespaceDTO>> entity = restTemplate.get(env,
          "/namespaces/find-by-item?itemKey={itemKey}&page={page}&size={size}", namespacePageDTO,
          itemKey, page, size);
    ...
    public void deleteAppNamespace(Env env, String appId, String namespaceName, String operator) {
      restTemplate.delete(env, "/apps/{appId}/appnamespaces/{namespaceName}?operator={operator}",
          appId, namespaceName, operator);
    }
  }
  ...
  public static class ItemAPI extends API {
    ...
    public PageDTO<ItemInfoDTO> getPerEnvItemInfoBySearch(Env env, String key, String value,
        int page, int size) {
      ResponseEntity<PageDTO<ItemInfoDTO>> entity = restTemplate.get(env,
    ...
    public ItemDTO loadItemByEncodeKey(Env env, String appId, String clusterName,
    ...
    public ItemDTO createCommentItem(String appId, Env env, String clusterName, String namespace,
        ItemDTO item) {
      return restTemplate.post(env,
          "apps/{appId}/clusters/{clusterName}/namespaces/{namespaceName}/comment_items", item,
          ItemDTO.class, appId, clusterName, namespace);
    }
  ...
  public static class AccessKeyAPI extends API {
    ...
    public AccessKeyDTO create(Env env, AccessKeyDTO accessKey) {
      return restTemplate.post(env, "apps/{appId}/accesskeys", accessKey, AccessKeyDTO.class,
          accessKey.getAppId());
    }
    ...
    public void delete(Env env, String appId, long id, String operator) {
      restTemplate.delete(env, "apps/{appId}/accesskeys/{id}?operator={operator}", appId, id,
          operator);
    }
    ...
    public void disable(Env env, String appId, long id, String operator) {
      restTemplate.put(env, "apps/{appId}/accesskeys/{id}/disable?operator={operator}", null, appId,
          id, operator);
    }
  }
  ...
  public static class ReleaseAPI extends API {
    ...
    public void rollbackTo(Env env, long releaseId, long toReleaseId, String operator) {
      restTemplate.put(env,
          "releases/{releaseId}/rollback?toReleaseId={toReleaseId}&operator={operator}", null,
          releaseId, toReleaseId, operator);
    }
  }
  ...
  public static class CommitAPI extends API {
    ...
    public List<CommitDTO> findByKey(String appId, Env env, String clusterName,
        String namespaceName, String key, int page, int size) {

      CommitDTO[] commitDTOs = restTemplate.get(env,
          "apps/{appId}/clusters/{clusterName}/namespaces/{namespaceName}/commit?key={key}&page={page}&size={size}",
          CommitDTO[].class, appId, clusterName, namespaceName, key, page, size);
  ...
  public static class ServerConfigAPI extends API {
    public List<ServerConfig> findAllConfigDBConfig(Env env) {
      return restTemplate.get(env, "/server/config/find-all-config",
          new ParameterizedTypeReference<List<ServerConfig>>() {}).getBody();
    }

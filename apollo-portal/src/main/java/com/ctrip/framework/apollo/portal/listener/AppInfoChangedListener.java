...
public class AppInfoChangedListener {
  ...
  public AppInfoChangedListener(
      final AdminServiceAPI.AppAPI appAPI, final PortalSettings portalSettings) {
    this.appAPI = appAPI;
  ...
  public void onAppInfoChange(AppInfoChangedEvent event) {
    ...
    for (Env env : envs) {
      try {
        ...
        Tracer.logError(
            String.format("Update app's info failed. Env = %s, AppId = %s", env, appId), e);
      }
    }
  }
}

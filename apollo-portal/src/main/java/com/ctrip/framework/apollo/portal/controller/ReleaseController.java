...
public class ReleaseController {
  ...
  public ReleaseDTO createGrayRelease(
      @PathVariable String appId,
      @PathVariable String env,
      @PathVariable String clusterName,
      @PathVariable String namespaceName,
  ...
  public List<ReleaseBO> findAllReleases(
      ...
      @Valid
          ...
          int page,
      @Valid @Positive(message = "size should be positive number") @RequestParam(defaultValue = "5")
          int size) {
  ...
  public ReleaseCompareResult compareRelease(
      @PathVariable String env,
  ...
  public void rollback(
    ...
    if (!permissionValidator.hasReleaseNamespacePermission(
        release.getAppId(), release.getNamespaceName(), env)) {
      throw new AccessDeniedException("Access is denied");
    }

    if (toReleaseId > -1) {
      releaseService.rollbackTo(
          Env.valueOf(env), releaseId, toReleaseId, userInfoHolder.getUser().getUserId());
    } else {

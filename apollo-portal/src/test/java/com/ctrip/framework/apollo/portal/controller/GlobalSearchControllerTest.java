...
public class GlobalSearchControllerTest {
  ...
  public void testGet_ItemInfo_BySearch_WithKeyAndValueAndActiveEnvs_ReturnEmptyItemInfos()
      throws Exception {
    when(globalSearchService.getAllEnvItemInfoBySearch(anyString(), anyString(), eq(0),
        eq(perEnvSearchMaxResults))).thenReturn(SearchResponseEntity.ok(new ArrayList<>()));
    mockMvc
        ...
        .andExpect(status().isOk()).andExpect(
            content().json("{\"body\":[],\"hasMoreData\":false,\"message\":\"OK\",\"code\":200}"));
    verify(portalConfig, times(1)).getPerEnvSearchMaxResults();
    verify(globalSearchService, times(1)).getAllEnvItemInfoBySearch(anyString(), anyString(), eq(0),
  ...
  public void testGet_ItemInfo_BySearch_WithKeyAndValueAndActiveEnvs_ReturnExpectedItemInfos_ButOverPerEnvLimit()
    ...
    when(globalSearchService.getAllEnvItemInfoBySearch(eq("query-key"), eq("query-value"), eq(0),
        eq(perEnvSearchMaxResults))).thenReturn(SearchResponseEntity

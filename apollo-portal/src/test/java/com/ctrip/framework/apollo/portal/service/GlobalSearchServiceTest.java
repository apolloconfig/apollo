...
public class GlobalSearchServiceTest {
  ...
  public void testGet_PerEnv_ItemInfo_BySearch_withKeyAndValue_ReturnExpectedItemInfos() {
    ...
    ItemInfoDTO itemInfoDTO =
        new ItemInfoDTO("TestApp", "TestCluster", "TestNamespace", "TestKey", "TestValue");
    List<ItemInfoDTO> mockItemInfoDTOs = new ArrayList<>();
    mockItemInfoDTOs.add(itemInfoDTO);
    Mockito.when(itemAPI.getPerEnvItemInfoBySearch(any(Env.class), eq("TestKey"), eq("TestValue"),
        eq(0), eq(1))).thenReturn(new PageDTO<>(mockItemInfoDTOs, PageRequest.of(0, 1), 1L));
    SearchResponseEntity<List<ItemInfo>> mockItemInfos =
        globalSearchService.getAllEnvItemInfoBySearch("TestKey", "TestValue", 0, 1);
    ...
    devMockItemInfos.add(new ItemInfo("TestApp", Env.DEV.getName(), "TestCluster", "TestNamespace",
        "TestKey", "TestValue"));
    proMockItemInfos.add(new ItemInfo("TestApp", Env.PRO.getName(), "TestCluster", "TestNamespace",
        "TestKey", "TestValue"));
    allEnvMockItemInfos.addAll(devMockItemInfos);
    allEnvMockItemInfos.addAll(proMockItemInfos);

    verify(itemAPI, times(2)).getPerEnvItemInfoBySearch(any(Env.class), eq("TestKey"),
        eq("TestValue"), eq(0), eq(1));
  ...
  public void testGet_PerEnv_ItemInfo_withKeyAndValue_BySearch_ReturnEmptyItemInfos() {
    ...
    activeEnvs.add(Env.PRO);
    Mockito.when(
        itemAPI.getPerEnvItemInfoBySearch(any(Env.class), anyString(), anyString(), eq(0), eq(1)))
        .thenReturn(new PageDTO<>(new ArrayList<>(), PageRequest.of(0, 1), 0L));
    SearchResponseEntity<List<ItemInfo>> result =
        globalSearchService.getAllEnvItemInfoBySearch("NonExistentKey", "NonExistentValue", 0, 1);
  ...
  public void testGet_PerEnv_ItemInfo_BySearch_withKeyAndValue_ReturnExpectedItemInfos_ButOverPerEnvLimit() {
    ...
    mockItemInfoDTOs.add(itemInfoDTO);
    Mockito.when(itemAPI.getPerEnvItemInfoBySearch(any(Env.class), eq("TestKey"), eq("TestValue"),
        eq(0), eq(1))).thenReturn(new PageDTO<>(mockItemInfoDTOs, PageRequest.of(0, 1), 2L));
    SearchResponseEntity<List<ItemInfo>> mockItemInfos =
        globalSearchService.getAllEnvItemInfoBySearch("TestKey", "TestValue", 0, 1);
    assertEquals(2, mockItemInfos.getBody().size());
    ...
    allEnvMockItemInfos.addAll(proMockItemInfos);
    String message =
        "In DEV , PRO , more than 1 items found (Exceeded the maximum search quantity for a single environment). Please enter more precise criteria to narrow down the search scope.";
    verify(itemAPI, times(2)).getPerEnvItemInfoBySearch(any(Env.class), eq("TestKey"),
        eq("TestValue"), eq(0), eq(1));
    verify(portalSettings, times(1)).getActiveEnvs();
    assertEquals(allEnvMockItemInfos.toString(), mockItemInfos.getBody().toString());

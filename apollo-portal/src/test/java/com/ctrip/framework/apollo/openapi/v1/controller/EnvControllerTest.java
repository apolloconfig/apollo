...
public class EnvControllerTest {
  ...
  @MockBean
  private EnvOpenApiService envOpenApiService;

  private UserInfo authenticatedUser;

  @Before
  public void setUpSecurityMocks() {
    when(unifiedPermissionValidator.hasCreateClusterPermission(Mockito.anyString()))
        .thenReturn(true);
  ...
  public void testGetEnvs() throws Exception {
    List<String> envs = Arrays.asList("DEV", "FAT");
    when(envOpenApiService.getEnvs()).thenReturn(envs);

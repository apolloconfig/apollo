...
public class ConsumerPermissionValidatorTest {
    ...
    /**
     * TC001: Consumer has specified permission, should return true
     */
    @Test
    public void testHasPermission_UserHasPermission_ReturnsTrue() {
        // Arrange
        when(permissionService.consumerHasPermission(CONSUMER_ID, PERMISSION_TYPE, TARGET_ID)).thenReturn(true);

        // Act
        boolean result = validator.hasPermission(TARGET_ID, PERMISSION_TYPE);

        // Assert
        assertTrue(result);
        verify(consumerAuthUtil, times(1)).retrieveConsumerIdFromCtx();
        verify(permissionService, times(1)).consumerHasPermission(CONSUMER_ID, PERMISSION_TYPE, TARGET_ID);
    }

    /**
     * TC002: Consumer does not have specified permission, should return false
     */
    @Test
    public void testHasPermission_UserHasNoPermission_ReturnsFalse() {
        // Arrange
        when(permissionService.consumerHasPermission(CONSUMER_ID, PERMISSION_TYPE, TARGET_ID)).thenReturn(false);
    ...
    /**
     * TC003: retrieveConsumerIdFromCtx throws exception, should throw exception
     */
    @Test
    public void testHasPermission_RetrieveConsumerIdThrowsException_ThrowsException() {
        // Arrange
        when(consumerAuthUtil.retrieveConsumerIdFromCtx()).thenThrow(new RuntimeException("Consumer ID not found"));

        // Act & Assert
        assertThrows(RuntimeException.class, () -> {
            validator.hasPermission(TARGET_ID, PERMISSION_TYPE);
        });
    ...
    /**
     * TC004: consumerHasPermission throws exception, should throw exception
     */
    @Test
    public void testHasPermission_ConsumerHasPermissionThrowsException_ThrowsException() {
        // Arrange
        when(permissionService.consumerHasPermission(CONSUMER_ID, PERMISSION_TYPE, TARGET_ID))
                .thenThrow(new RuntimeException("Permission check failed"));

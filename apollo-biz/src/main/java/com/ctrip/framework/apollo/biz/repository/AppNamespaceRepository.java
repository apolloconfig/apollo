...
public interface AppNamespaceRepository extends PagingAndSortingRepository<AppNamespace, Long>{
  ...
  @Modifying
  @Query("UPDATE AppNamespace SET IsDeleted = 1, DeletedAt = UNIX_TIMESTAMP(), DataChange_LastModifiedBy = ?2 WHERE AppId=?1")
  int batchDeleteByAppId(String appId, String operator);

  @Modifying
  @Query("UPDATE AppNamespace SET IsDeleted = 1, DeletedAt = UNIX_TIMESTAMP(), DataChange_LastModifiedBy = ?3 WHERE AppId=?1 and Name = ?2")

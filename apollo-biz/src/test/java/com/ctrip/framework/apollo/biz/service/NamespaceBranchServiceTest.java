...
public class NamespaceBranchServiceTest extends AbstractIntegrationTest {
  ...
  @Autowired private NamespaceBranchService namespaceBranchService;

  @Autowired private ReleaseHistoryService releaseHistoryService;
  ...
  @Test
  @Sql(
      scripts = "/sql/namespace-branch-test.sql",
  ...
  public void testUpdateBranchGrayRulesWithUpdateOnce() {
    ...
    namespaceBranchService.updateBranchGrayRules(
        testApp, testCluster, testNamespace, testBranchName, rule);

    GrayReleaseRule activeRule =
        namespaceBranchService.findBranchGrayRules(
            testApp, testCluster, testNamespace, testBranchName);
    ...
    Page<ReleaseHistory> releaseHistories =
        releaseHistoryService.findReleaseHistoriesByNamespace(
  ...
  public void testUpdateBranchGrayRulesWithUpdateTwice() {
    ...
    namespaceBranchService.updateBranchGrayRules(
        testApp, testCluster, testNamespace, testBranchName, secondRule);
  ...
  public void testUpdateRulesReleaseIdWithOldRuleNotExist() {
    ...
    namespaceBranchService.updateRulesReleaseId(
        testApp, testCluster, testNamespace, testBranchName, latestReleaseId, operator);
  ...
  public void testDeleteBranch() {
    ...
    namespaceBranchService.deleteBranch(
        ...
        testNamespace,
        testBranchName,
        NamespaceBranchStatus.DELETED,
        operator);

    Namespace branch = namespaceBranchService.findBranch(testApp, testCluster, testNamespace);
    Assert.assertNull(branch);

    GrayReleaseRule latestRule =
        namespaceBranchService.findBranchGrayRules(
            testApp, testCluster, testNamespace, testBranchName);

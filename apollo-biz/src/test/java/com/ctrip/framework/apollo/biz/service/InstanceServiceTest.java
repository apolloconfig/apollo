...
public class InstanceServiceTest extends AbstractIntegrationTest {
  ...
  public void testCreateAndFindInstance() throws Exception {
    ...
    instanceService.createInstance(
        assembleInstance(someAppId, someClusterName, someDataCenter, someIp));
  ...
  public void testFindInstancesByIds() throws Exception {
    ...
    Instance anotherInstance =
        instanceService.createInstance(
            assembleInstance(someAppId, someClusterName, someDataCenter, anotherIp));

    List<Instance> instances =
        instanceService.findInstancesByIds(
            Sets.newHashSet(someInstance.getId(), anotherInstance.getId()));
  ...
  public void testCreateAndFindInstanceConfig() throws Exception {
    ...
    InstanceConfig instanceConfig =
        instanceService.findInstanceConfig(
            someInstanceId, someConfigAppId, someConfigNamespaceName);

    assertNull(instanceConfig);

    instanceService.createInstanceConfig(
  ...
  public void testFindActiveInstanceConfigs() throws Exception {
    ...
    prepareInstanceConfigForInstance(
        someInstanceId,
        someConfigAppId,
        someConfigClusterName,
        someConfigNamespaceName,
  ...
  public void testFindInstancesByNamespace() throws Exception {
    ...
    Page<Instance> result =
        instanceService.findInstancesByNamespace(
  ...
  public void testFindInstancesByNamespaceAndInstanceAppId() throws Exception {
    ...
    Instance anotherInstance =
        instanceService.createInstance(
            assembleInstance(anotherAppId, someClusterName, someDataCenter, someIp));
    ...
    Page<Instance> result =
        instanceService.findInstancesByNamespaceAndInstanceAppId(
            someAppId,
            someConfigAppId,
            someConfigClusterName,
            someConfigNamespaceName,
  ...
  public void testFindInstanceConfigsByNamespaceWithReleaseKeysNotIn() throws Exception {
    ...
    List<InstanceConfig> instanceConfigs =
        instanceService.findInstanceConfigsByNamespaceWithReleaseKeysNotIn(
            someConfigAppId,
            someConfigClusterName,
            someConfigNamespaceName,
            Sets.newHashSet(anotherReleaseKey, yetAnotherReleaseKey));

    assertEquals(Lists.newArrayList(someInstanceConfig, anotherInstanceConfig), instanceConfigs);
  }

  private InstanceConfig prepareInstanceConfigForInstance(
      long instanceId,
      String configAppId,
      String configClusterName,
      String configNamespace,
      String releaseKey,
      Date lastModifiedTime) {

    InstanceConfig someConfig =
        assembleInstanceConfig(
            instanceId, configAppId, configClusterName, configNamespace, releaseKey);
  ...
  private Instance assembleInstance(
      String appId, String clusterName, String dataCenter, String ip) {

...
public class ReleaseCreationTest extends AbstractIntegrationTest {
  ...
  @Test
  @Sql(
      scripts = "/sql/release-creation-test.sql",
  ...
  public void testPublishMasterNamespaceAndBranchHasNotItems() {
    ...
    Assert.assertEquals(
        ReleaseOperation.MASTER_NORMAL_RELEASE_MERGE_TO_GRAY, branchReleaseHistory.getOperation());
    Assert.assertEquals(latestChildNamespaceRelease.getId(), branchReleaseHistory.getReleaseId());
    Assert.assertTrue(
        branchReleaseHistory
            .getOperationContext()
  ...
  public void testRollback() {
    ...
    Assert.assertEquals(
        ReleaseOperation.MATER_ROLLBACK_MERGE_TO_GRAY, branchReleaseHistory.getOperation());
    Assert.assertEquals(
        childNamespaceNewLatestRelease.getId(), branchReleaseHistory.getReleaseId());
    Assert.assertEquals(8, branchReleaseHistory.getPreviousReleaseId());
    Assert.assertTrue(
        branchReleaseHistory
            .getOperationContext()
            .contains(
                String.format("\"baseReleaseId\":%d", parentNamespaceNewLatestRelease.getId())));
  }

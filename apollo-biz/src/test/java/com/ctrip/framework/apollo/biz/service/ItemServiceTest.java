...
public class ItemServiceTest extends AbstractIntegrationTest {
  ...
  @Test
  @Sql(scripts = {"/sql/namespace-test.sql", "/sql/item-test.sql"},
  ...
  public void testSaveItemWithNamespaceValueLengthLimitOverride() {
    ...
    try {
      ...
      Assert.assertTrue(
          e instanceof BadRequestException && e.getMessage().contains("value too long"));
    }
  }
  ...
  public void testSearchItem() {
    ...
    String itemKey = "k1";
    String itemValue = "v1";
    Page<ItemInfoDTO> ExpectedItemInfoDTOSByKeyAndValue =
        itemService.getItemInfoBySearch(itemKey, itemValue, PageRequest.of(0, 200));
    Page<ItemInfoDTO> ExpectedItemInfoDTOSByKey =
        itemService.getItemInfoBySearch(itemKey, "", PageRequest.of(0, 200));
    Page<ItemInfoDTO> ExpectedItemInfoDTOSByValue =
        itemService.getItemInfoBySearch("", itemValue, PageRequest.of(0, 200));
    Assert.assertEquals(itemInfoDTO.toString(),
        ExpectedItemInfoDTOSByKeyAndValue.getContent().get(0).toString());
    Assert.assertEquals(itemInfoDTO.toString(),
        ExpectedItemInfoDTOSByKey.getContent().get(0).toString());
    Assert.assertEquals(itemInfoDTO.toString(),
